...
function NavLink({ href, label }: { href: string; label: string }) {
  ...
  return (
    <Link
      href={href}
      className={`block rounded-lg px-3 py-2 text-sm hover:text-white hover:bg-white/5 ${isActive ? 'bg-white/10 text-white' : 'text-white/80'}`}
        ...
        <aside className="border-r border-white/10 hidden md:block">
          <div className="p-4">
            ...
            <nav className="space-y-1">
              <NavLink href="/dashboard-preview" label="Dashboard" />
              <NavLink href="/dashboard-preview/calls" label="Calls & Recordings" />
              <NavLink href="/dashboard-preview/appointments" label="Appointments" />
              <NavLink href="/dashboard-preview/integrations" label="Integrations" />
              <NavLink href="/dashboard-preview/billing" label="Billing & Plan" />
              <NavLink href="/dashboard-preview/settings" label="Settings" />
            </nav>
          </div>
        </aside>
        {/* Main */}
            ...
            <div className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-md bg-white text-black px-3 py-1.5 text-sm font-medium hover:bg-white/90"
                >
                  Test Call
                </button>
                <button
                  onClick={() => setLive(v => !v)}
                  className="rounded-md border border-white/20 px-3 py-1.5 text-sm text-white/80 hover:text-white"
                >
                  {live ? 'Pause' : 'Go Live'}
                </button>
