...
export default function SettingsPage() {
  ...
  function saveAll() { setFlash("Settings saved (preview)"); setTimeout(() => setFlash(null), 1500); }
  function sendInvite() { if (!inviteEmail) return; setFlash(`Invite sent to ${inviteEmail} as ${inviteRole}`); setInviteEmail(""); setTimeout(() => setFlash(null), 1500); }
      ...
      <div className="mt-6 grid gap-4 grid-cols-[repeat(auto-fit,minmax(320px,1fr))]">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-3 space-y-3 text-sm">
            <label className="block">Company name<input value={company} onChange={e=>setCompany(e.target.value)} className="mt-1 w-full bg-white/5 border border-white/10 rounded-md px-3 py-2" /></label>
            <label className="block">Time zone<input value={tz} onChange={e=>setTz(e.target.value)} className="mt-1 w-full bg-white/5 border border-white/10 rounded-md px-3 py-2" /></label>
          </div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 md:col-span-2">
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="min-w-[560px] w-full text-sm">
              <thead className="text-white/60"><tr><th className="text-left font-normal">Day</th><th className="text-left font-normal">Open</th><th className="text-left font-normal">Start</th><th className="text-left font-normal">End</th></tr></thead>
              <tbody>
                {days.map(d => (
                  <tr key={d} className="border-b border-white/10">
                    <td className="py-2">{d}</td>
                    <td><input type="checkbox" checked={matrix[d].open} onChange={e=>updateDay(d,{open:e.target.checked})} /></td>
                    <td><input type="time" value={matrix[d].start} onChange={e=>updateDay(d,{start:e.target.value})} className="bg-white/5 border border-white/10 rounded-md px-2 py-1" /></td>
                    <td><input type="time" value={matrix[d].end} onChange={e=>updateDay(d,{end:e.target.value})} className="bg-white/5 border border-white/10 rounded-md px-2 py-1" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-4 grid gap-3 grid-cols-[repeat(auto-fit,minmax(220px,1fr))]">
            <label className="flex items-center gap-2 text-sm text-white/80"><input type="checkbox" checked={afterHours.transfer} onChange={e=>setAfterHours(s=>({...s, transfer:e.target.checked}))} />After‑hours: transfer to on‑call</label>
            <label className="flex items-center gap-2 text-sm text-white/80"><input type="checkbox" checked={afterHours.voicemail} onChange={e=>setAfterHours(s=>({...s, voicemail:e.target.checked}))} />After‑hours: send to voicemail</label>
          </div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-3 grid gap-4 grid-cols-[repeat(auto-fit,minmax(260px,1fr))]">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <select value={inviteRole} onChange={e=>setInviteRole(e.target.value)} className="bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm"><option>Admin</option><option>Viewer</option></select>
