...
export default function AuthClient({ initialTab }: { initialTab: PanelMode }) {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        console.warn("marketing_auth_popup_close_failed", { message: (error as Error)?.message });
      } finally {
        popupRef.current = null;
      }
      clearGooglePopupMonitor();
  ...
  return (
    <div className="bg-neutral-950 text-white">
      <section className="mx-auto max-w-lg px-6 pb-16 pt-12 md:pb-24 md:pt-16">
        ...
        <div id="panel-login" role="tabpanel" aria-labelledby="tab-login" hidden={tab !== "login"} className="mt-8">
          {tab === "login" && (
            <>
              ...
              <form onSubmit={handleLoginSubmit} className="mt-8 space-y-4" aria-label="Email and password sign-in">
        ...
        <div id="panel-signup" role="tabpanel" aria-labelledby="tab-signup" hidden={tab !== "signup"} className="mt-8">
          {tab === "signup" && (
            <>
              ...
              <form className="mt-8 space-y-4" onSubmit={handleSignupSubmit} aria-label="Create account">
                <label className="block text-xs font-medium uppercase tracking-wide text-white/60">
                  Work email
                  <input
                    name="email"
